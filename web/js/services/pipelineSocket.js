...
import { API_PREFIX, apiFetch } from "../shared/api.js";
...
export function connectProgress(slug, { onEvent, onError, onClose } = {}) {
  ...
  const url = `${proto}//${window.location.host}${API_PREFIX}/pipeline/progress/${encodeURIComponent(slug)}`;
...
// 需要确认时返回 pending 载荷，否则 null。刻意使用 apiFetch（自动拼接前缀）—
// 此处 404/non-ok 是「无待处理项」的正常情况，不应抛错。
export async function fetchPendingKind(slug) {
  try {
    const res = await apiFetch(`/pipeline/packs/${encodeURIComponent(slug)}/pending-kind`);
